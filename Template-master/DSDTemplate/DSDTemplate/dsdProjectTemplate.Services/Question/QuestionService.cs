...
{
    public class QuestionService :BaseServiceClass, IQuestionService
    {
       
        IActionsHistoryService _actionsHistoryService = new ActionsHistoryService();
     
        string _serviceFor = "question";
        public QuestionService()
        {
            
        }

        public async Task<ResponseModel> AddAsync(QuestionViewModel request)
        {
            ...
            try
            {
                ...
                var _existRecordResponse = await CheckIfRecordIsExist(AppTable.SecurityQuestions.ToString(), "Question", request.Question, request.Id);
                ...
                {
                    var query = "INSERT INTO " + AppTable.SecurityQuestions + " (Question,IsActive,CreatedBy,CreatedDate) VALUES (@Question, @IsActive,@CreatedBy,@CreatedDate)";
                    ...
                    await _actionsHistoryService.AddAsync(new ActionsHistoryViewModel
                    {
                        ...
                        TableName = AppTable.SecurityQuestions.ToString(),
        ...
        public async Task<ResponseModel> UpdateAsync(QuestionViewModel request)
        {
            ...
            try
            {
                ...
                if (item != null)
                {
                    ...
                    using (IDbConnection con = new SqlConnection(SQLConnectionString.dbConnection))
                    {
                        var query = "UPDATE " + AppTable.SecurityQuestions + " SET  Question=@Question,IsActive=@IsActive,UpdatedBy=@UpdatedBy,UpdatedDate=@UpdatedDate WHERE Id=@Id";
        ...
        public async Task<IEnumerable<QuestionViewModel>> GetAllAsync()
        {
            try
            {
                using (var con = new SqlConnection(SQLConnectionString.dbConnection))
                {
                    ...
                    var query = "select * from " + AppTable.SecurityQuestions + " (nolock) order by id desc ";
                    var parameters = new DynamicParameters();
                    IEnumerable<QuestionViewModel> response = await con.QueryAsync<QuestionViewModel>(query, parameters, commandType: CommandType.Text);
                    con.Close();
                    return response;
                }
            }
        ...
        public async Task<List<SelectListItem>> GetDropListAsync()
        {
            try
            {
                ...
                using (var con = new SqlConnection(SQLConnectionString.dbConnection))
                {
                    var query = "select Id,Question from " + AppTable.SecurityQuestions + " (nolock) where    IsActive=1 order by Question asc ";
                    var parameters = new DynamicParameters();
                    var _data = await con.QueryAsync<QuestionViewModel>(query, parameters, commandType: CommandType.Text);
                    con.Close();
                    _listData.AddRange(_data.Select(g => new SelectListItem { Text = g.Question, Value = g.Id.ToString() }).ToList());
                    return _listData;
                }
        ...
        private async Task<QuestionViewModel> GetByIdAsync(long Id)
        {
            QuestionViewModel response = new QuestionViewModel();
            try
            {
                using (var con = new SqlConnection(SQLConnectionString.dbConnection))
                {
                    var query = "select * from " + AppTable.SecurityQuestions + " (nolock) where Id=@Id ";
                    var parameters = new DynamicParameters();
                    parameters.Add("@Id", Id);
                    response = await con.QueryFirstOrDefaultAsync<QuestionViewModel>(query, parameters, commandType: CommandType.Text);
                    con.Close();
                    return response;
                }
