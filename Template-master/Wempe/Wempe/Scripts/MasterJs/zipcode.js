var TempValues = { StateID: '', CountyId: '', CityId:'' };
var _lock = true;
...
function getList(pageNo) {
    ...
    $.ajax({
        url: siteUrl + 'Zipcode/getZipcodeList',
...
function Add() {
    ...
    $.ajax({
        url: siteUrl + 'Zipcode/AddZipcode',
        type: 'POST',
        headers: { "requestType": "client" },
        data: JSON.stringify(model),
        contentType: 'application/json;charset=utf-8',
...
function Edit(id, stateId, countryId) {
    ...
    jQuery.ajax({
        url: siteUrl + 'Zipcode/EditZipcode',
        dataType: 'json',
        headers: { "requestType": "client" },
        data: { "id": id },
        contentType: 'application/json;charset=utf-8',
        success: function (data) {
           ...
           // $('#txtName').val(data.city);
            ...
            $('#txtPostalCode').val(data.ZipCode);

            TempValues.StateID = data.StateId;
            TempValues.CountyId = data.CountyID;
            TempValues.CityId = data.city;



            $('#drpCityCountry').val(data.CountryId);
...
function bindCity(drpCity, id) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
          ...
          //  alert(TempValues.CityId);
            if (TempValues.CityId != '') {
                drpCity.val(TempValues.CityId);
                TempValues.CityId = '';
            }
...
function bindCounty(drpCounty, id) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            bindCity($("#drpcityCity"), $('#drpcityCounty').val());
        },
        error: function (data) {
            drpCounty.empty().append('<option selected="selected" value="">Please select</option>');
            showMessage('Oops', 'error', data.responseJSON.Message, 'toast-top-right');
...
$(document).ready(function () {
    $('#drpSearchCountry').change(function () {
        ...
        $('#drpSearchCityCounty').empty().append('<option selected="selected" value="">Please Select</option>');
        $('#drpSearchCityCity').empty().append('<option selected="selected" value="">Please Select</option>');
    ...
    $('#drpSearchCityCounty').change(function () {
        ...
        if ($('#drpSearchCityCounty').val() != '') {
            bindCity($("#drpSearchCityCity"), $('#drpSearchCityCounty').val());
        }
        getList(1);
    });
    ...
    $('#drpSearchCityCity').change(function () {
    ...
    $('#drpCityCountry').change(function () {
        ...
        $('#drpcityCounty').empty().append('<option selected="selected" value="">Please Select</option>');
        $('#drpcityCity').empty().append('<option selected="selected" value="">Please Select</option>');
