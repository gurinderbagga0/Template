...
$('#drpBrandItemSearch').change(function () {
    getList(1);
});
...
function Add() {
    ...
    var model =
    {
        ...
        item: $('#txtName').val(),
        brandId: $('#drpBrandItemAdd').val(),
        IsActive: $('#chkIsActive').is(':checked')
    };


    $.ajax({
        url: siteUrl + 'Master/AddItem',
        type: 'POST',
        headers: { "requestType": "client" },
        data: JSON.stringify(model),
        contentType: 'application/json;charset=utf-8',
...
function Edit(id) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            $('#hdnid').val(data.Id);
            $('#txtName').val(data.item);
            $('#drpBrandItemAdd').val(data.brandId);
