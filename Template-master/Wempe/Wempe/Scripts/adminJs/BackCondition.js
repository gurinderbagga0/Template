...
function getList(pageNo) {
    ...
    $.ajax({
        url: siteUrl + 'BackCondition/getList',
...
function Add() {
    ...
    var model =
    {
        BackConditionID: parseInt($('#hdnid').val()),
        BackCondition: $('#txtName').val(),
...
function Edit(id) {  
    ...
    jQuery.ajax({
        url: siteUrl + 'BackCondition/Edit',
        dataType: 'json',
        headers: { "requestType": "client" },
        data: { "id": id },
        contentType: 'application/json;charset=utf-8',
        success: function (data) {
            ...
            $('#hdnid').val(data.BackConditionID);
            $('#txtName').val(data.BackCondition);
