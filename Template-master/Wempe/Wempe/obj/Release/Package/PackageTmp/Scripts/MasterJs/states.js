...
function getList(pageNo) {
    ...
    $.ajax({
        url: siteUrl + 'Master/getStateList',
        ...
        success: function (data) {
            ...
            if (data.length == 0) {
                ...
                //$('#tbodyTemplate').tmpl('<>').appendTo('#tbodyDetails');
...
function Add() {
    ...
    var model =
    {
        ...
        stateFullName: $('#txtFullName').val(),
        state: $('#txtName').val(),
        CountryId: $('#drpSateCountry').val(),
        IsActive: $('#chkIsActive').is(':checked')
    };


    $.ajax({
        url: siteUrl + 'Master/AddState',
        type: 'POST',
        headers: { "requestType": "client" },
        data: JSON.stringify(model),
        contentType: 'application/json;charset=utf-8',
...
function Edit(id) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            $('#hdnid').val(data.Id);
            $('#txtName').val(data.state);
            $('#txtFullName').val(data.stateFullName);
            $('#drpSateCountry').val(data.CountryId);
            if (data.IsActive) {
