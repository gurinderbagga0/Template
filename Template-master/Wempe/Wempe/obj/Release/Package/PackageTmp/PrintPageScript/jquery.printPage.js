...
(function( $ ){
  $.fn.printPage = function(options) {
    ...
    var pluginOptions = {
      attr : "href",
      url : false,
      message: "Please wait while we print your document" 
    };
    ...
    function addIframeToPage(el, pluginOptions){
        ...
        if (url.indexOf("#CallType") >= 0) {
            url = url.replace('#CallType', $('#hdnCallType').val());
            url = url.replace('#priAddress', $('#hdnPriAddress').val());
            url = url.replace('#hdnRepairNumber', $('#hdnRepairNumber').val());

            if ($('#chkPrintOnLetterHead').is(':checked')) {
                url = url.replace('#withLetterHead', "1");
            }
            else {
                url = url.replace('#withLetterHead', "0");
            }
