...
{
    [RoutePrefix("StaffLevelReport")]
    [Authorize()]

    public class StaffLevelReportController : Controller
    {
        // GET: Country
        IStaffLevelReportService _StaffLevelReportService;
        IOrganizationsService _OrganizationsService;
        IEmployeeService _EmployeeService;
        public StaffLevelReportController()
        {
            _StaffLevelReportService = new StaffLevelReportService();
        ...
        public ActionResult GetEEOJobCategory(int? organization)
        {
            try
            {
                var model = _StaffLevelReportService.BindEEOJobCategoryDropDown(organization);
        ...
        public ActionResult GetEEOProgramOffice(int? organization, int? filesubmission)
        {
            try
            {
                var model = _StaffLevelReportService.BindEEOProgramOfficeDropDown(organization.Value, filesubmission.Value);
        ...
        public ActionResult GetStaffLevelReport(int? organization, int? filesubmission, int? eeojobcategory, string eeoprogramoffice, string region)
        {
            try
            { 
                var modelStaffLevelReport = _StaffLevelReportService.GetStaffLevelReportService(organization, filesubmission, eeojobcategory, eeoprogramoffice, region);
                return PartialView("~/Views/StaffLevelReport/Partials/StaffLevelReport.cshtml", modelStaffLevelReport);
            }
            catch
            {
                return RedirectToAction("Errorwindow", "Home");
        ...
        [AllowAnonymous()]
        public ActionResult ExportStaffLevelReport(int? organization, int? filesubmission, int? eeojobcategory, string eeoprogramoffice, string region)
        {
            try
            { 
                var modelStaffLevelReport = _StaffLevelReportService.GetStaffLevelExportService(organization, filesubmission, eeojobcategory, eeoprogramoffice, region);
                return View(modelStaffLevelReport); 
            }
            catch
            {
                return RedirectToAction("Errorwindow", "Home");
        ...
        public ActionResult ExportToPDF(int? organization, int? filesubmission, int? eeojobcategory, string eeoprogramoffice, string region)
        {
            ...
            string url = string.Format(ConfigurationManager.AppSettings["AppUrl"] + "/StaffLevelReport/ExportStaffLevelReport?organization={0}&filesubmission={1}&eeojobcategory={2}&eeoprogramoffice={3}&region={4}", organization, filesubmission, eeojobcategory, eeoprogramoffice, region);
        ...
        private string RenderViewAsString(string viewName, StaffLevelReportModel model)
        {
